...
const DESCRIPTION =
  'Search local files, documents, and unstructured data on disk using the Sirchmunk MCP tool. Use when searching documentation, parsing complex document formats (PDF, Word, Excel, PPTX, etc.), querying local knowledge bases, or locating codebase information by content and semantic meaning.';

function registerBundledSkills(ctx: HostPluginContext): void {
  ...
  const provider = {
    ...
    async get(selected: { name: string }) {
